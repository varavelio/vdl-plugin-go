...
import type { GeneratedFile, GeneratorContext } from "../internal-types";
import {
  canEmitConst,
...
/**
 * Generates the Go file containing constants and composite values.
 */
export function generateConstantsFile(
  ...
  for (const constant of context.constantDescriptors) {
    ...
    if (canEmitConst(constant.def.typeRef, context, constant.def.position)) {
      ...
      g.line(
        `var ${constant.goName} = ${renderTypedValueExpression(
